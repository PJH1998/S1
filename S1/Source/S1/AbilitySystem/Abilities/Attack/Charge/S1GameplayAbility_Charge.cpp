...
#include "AbilitySystem/Abilities/Attack/Charge/S1GameplayAbility_Charge.h"
#include "Character/S1Character.h"
#include "NiagaraComponent.h"

#include "S1LogChannels.h"

void US1GameplayAbility_Charge::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void US1GameplayAbility_Charge::OnProgressionCycleCompleted(FGameplayTag EffectTag, int32 CycleCount, int32 MaxCycleCount)
{
	...
	LOG_WARNING(TEXT("COUNT [%d], RATIO [%f]."), CycleCount, Ratio);
	...
	if (MaxCycleCount > 0 && CycleCount >= MaxCycleCount)
	{
		...
		// EndAttachedEffect의 즉시 Deactivate로 취소되어 마지막 버스트가 아예 안 뜸(3번째 이펙트가 안 보이던 원인)
